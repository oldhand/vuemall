...
<script>
  ...
  import {types,get_billwaters} from './billwater'   
  ...
  export default {
	...
	mounted() { 
	         var that = this;   
			 get_billwaters(this.page).then( res => {  
			 	console.log('___________get_billwaters_________' + JSON.stringify(res));  
				that.waittext = '';
    ...
    methods: {
	       makebill(res)
		   {
				...
				for (var i in res)
				{
					var billwater_info = res[i];  
				    var billwatertype = billwater_info.billwatertype;  
				    var amount = billwater_info.amount;
				    var money =  billwater_info.money;  
					var published = billwater_info.published;
					var date = date_format(todatetime(published),'YYYY-MM-DD'); 
					for (var j in types)
					{
							if (billwatertype == types[j]['key'])
							{
									...
									for (var k in this.items)
									{
										if (this.items[k].date == date)
										{
												if (type == '+')
												{
													this.items[k].list.push({type:type,desc:'¥'+money,title:label,amount:'¥'+number_format(Math.abs(amount), 2, ".", ",")});
									...
									if (!hasdate)
									{
										if (type == '+')
										{
											this.items.push({date:date,list:[{type:type,desc:'¥'+money,title:label,amount:'¥'+number_format(Math.abs(amount), 2, ".", ",")}]}); 
										}
										else
										{
											this.items.push({date:date,list:[{type:type,desc:'¥'+money,title:label,amount:'¥'+number_format(Math.abs(amount), 2, ".", ",")}]});  
          ...
          onloading() {
	               ...
	               this.showLoading = 'show'; 
	  			   ...
	  			   get_billwaters(this.page).then( res => {  
				        console.log('___________get_billwaters___'+that.page+'______' + JSON.stringify(res));  
