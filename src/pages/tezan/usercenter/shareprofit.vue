...
<script> 
  ...
  import {get_shareprofits} from './shareprofit'   
  ...
  export default {
    ...
    data () {
      return { 
		   ...
		   items:[],  
           columns: [ 
			  {field: 'published', title: '分享时间',width: 500, headeralign:'center',columnalign:'center',nowrap:true},
			  {field: 'amount', title: '金额', width: 250, headeralign:'center',columnalign:'center',nowrap:true,color:'#b4282d'},  
          ]
      }
    },
	mounted() { 
	         var that = this;   
			 get_shareprofits(this.page).then( res => {  
			 	console.log('___________get_shareprofits_________' + JSON.stringify(res));  
				this.items = res; 
				that.waittext = '';
    ...
    methods: { 
          onloading() {
				   ...
				   this.showLoading = 'show'; 
	  			   ...
	  			   get_shareprofits(this.page).then( res => {  
				        console.log('___________get_shareprofits___'+that.page+'______' + JSON.stringify(res));  
